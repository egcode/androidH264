package com.example.eugene.applik;
...
public class MainActivity extends Activity implements SurfaceHolder.Callback, PreviewCallback
{
    ...
    private final static int DEFAULT_FRAME_RATE = 60;
...
//            this.startCamera();
        ...
        File file = new File(this.getFilesDir(), "eugene.h264");
        if(file.exists()) {
            file.delete();
        }
        try {
        ...
        if (width == 0) {
            this.encoder.init(176, 144, DEFAULT_FRAME_RATE, DEFAULT_BIT_RATE);
        } else {
